...
if (!telegramToken || !telegramChannel) {
  ...
  console.error(
    "Please ensure TELEGRAM_BOT_TOKEN and TELEGRAM_CHANNEL_ID are set in .env file",
  );
  process.exit(1);
}
...
class InstamartScraper {
  ...
  private async navigateToCategory(
    ...
    try {
      await page.locator(`button[aria-label="${categoryName}"]`).click();
      ...
      if (subCategories.length > 100) {
        ...
        for (const subCategory of subCategories) {
          try {
            ...
            console.error(
              `Error processing sub-category ${subCategory}:`,
              error,
            );
          }
        }
      ...
      throw new Error(
        `Failed to navigate to category ${categoryName}: ${error}`,
      );
    }
  }
  ...
  private async captureProductDetails(
    ...
    const sanitizedName = productDetails.name
      .replace(/[^a-z0-9]/gi, "_")
      .toLowerCase();
  ...
  private async scanProductsInView(
    page: Page,
    ...
    for (const discountElement of discountElements) {
      ...
      if (discountPercentage >= this.config.minDiscountPercentage) {
        ...
        const productId =
          (productDetails?.name?.replace(/[^a-z0-9]/gi, "_")
  ...
  private async getDiscountedProducts(
    ...
    while (
      ...
      const newProducts = await this.scanProductsInView(
        page,
        processedProducts,
      );
  ...
  private async scanLocation(location: Location): Promise<ProductScanResult[]> {
    ...
    try {
      ...
      try {
        await page
          .locator('[data-testid="set-gps-button"]')
          .click({ force: true });
      } catch (error) {
...
async function runWithRetry() {
  while (true) {
    try {
      ...
      console.log('\n=== Scan complete. Waiting 5 minutes before next scan... ===');
